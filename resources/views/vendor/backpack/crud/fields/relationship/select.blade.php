@php
    ...
    // Prefix before printing values
    $field['prefix'] = $field['prefix'] ?? '';

    // Checking for what elements to add prefix
    $field['prefix_keys'] = $field['prefix_keys'] ?? false;
    ...
    @if (count($field['options']))
        @foreach ($field['options'] as $key => $option)
            @php
                ...
                $prefix = '';
                if($field['prefix_keys'] !== false) {
                    if(in_array($key, $field['prefix_keys'])) {
                       $prefix = $field['prefix'];
                    }
                }
            @endphp

            <option value="{{ $key }}" {{$selected}}>{{ $prefix . $option }}</option>
        @endforeach
    @endif
</select>
