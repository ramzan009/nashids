    		...
    		<div class="file-preview">
		        ...
		        @else
                    @php
                       ...
                       $fileName = count($fileName) != 1 ? $fileName[count($fileName)-1] : $fileName;
                    @endphp
		            <a target="_blank" href="{{ isset($field['disk'])?asset(\Storage::disk($field['disk'])->url($file_path)):asset('storage/'.$file_path) }}">{{$fileName}}</a>
		        @endif
...
{{-- Extra CSS and JS for this particular field --}}
	...
	<style type="text/css">
		...
		.backstrap-file-input:lang(ru) ~ .backstrap-file-label::after {
			content: "Выбрать";
		}
